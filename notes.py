...
# Configuration
base_path = r"C:\Users\anton\AppData\Local\Programs\Microsoft VS Code\notes"
...
<body>
    ...
    <script type="text/babel">
        ...
        const IconBell = ({ size=20 }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>;
        ...
        const NoteWindow = ({ note, isOwner, viewMode, canvasOffset, onMinimize, onDelete, onUpdate, onFocus, zIndex, onNotify, onViewProfile, isSaved, onToggleSave }) => {
            const [localPos, setLocalPos] = useState({ x: 0, y: 0 });
            const [isDragging, setIsDragging] = useState(false);
            ...
            return (
                <div 
                    className={`absolute flex flex-col glass-window ${isClosing ? 'animate-close' : 'animate-open'} ${isDragging ? 'dragging shadow-2xl' : 'smooth-move'}`}
                ...
                >
                    <div onMouseDown={handleDragStart} onTouchStart={handleDragStart} className="h-14 bg-white/50 border-b border-white/60 flex items-center justify-between px-3 cursor-move select-none">
                        <div className="flex items-center gap-3 pl-1 text-slate-600">
                            ...
                            <div className="flex flex-col pointer-events-none">
                                {note.authorName && <span className="text-[10px] font-bold text-slate-700 leading-tight">{note.authorName}</span>}
                        ...
                        <div className="flex items-center gap-1.5 pl-2">
                            {isOwner ? (
                                ...
                                <>
                                    <button onClick={() => onToggleSave(note.id)} className={`p-1.5 rounded transition-colors ${isSaved ? 'text-rose-500 bg-rose-50' : 'text-slate-400 hover:bg-rose-50 hover:text-rose-400'}`} title={isSaved ? "Unsave" : "Save"}><IconHeart size={14} fill={isSaved} /></button>
                                    <button onClick={handleShare} className="p-1.5 hover:bg-indigo-100 text-slate-500 rounded"><IconShare size={14} /></button>
                    ...
                    <div className="flex-1 relative bg-white/30 overflow-hidden">
                        <textarea 
                            readOnly={!isOwner}
                            className="w-full h-full bg-transparent p-4 outline-none resize-none text-slate-700 placeholder-slate-400 text-sm leading-relaxed"
        ...
        const App = () => {
            ...
            const [authInput, setAuthInput] = useState({ email: "", password: "", name: "", avatar: "🌱" });
            ...
            const [isSettingCloud, setIsSettingCloud] = useState(false);
            ...
            const visibleNotes = useMemo(() => {
                ...
                if (view === 'HOME') {
                    ...
                    // AND are less than 24 hours old (24 * 60 * 60 * 1000 = 86400000 ms)
                    const now = Date.now();
                    const ONE_DAY = 86400000;
                    filtered = notes.filter(n => n.published && (n.reports || 0) < 10 && (now - n.id < ONE_DAY));
                }
            ...
            const createNote = (x, y, lat, lng, source) => {
                ...
                const newNote = {
                    ...
                    authorAvatar: userAccount ? userAccount.avatar : "👤",
                    history: [],
                    published: false, reports: 0
                };
            ...
            const handleSearchChange = (e) => {
                ...
                if (val.length > 1) setSearchSuggestions(Object.keys(CITIES).filter(c => c.toLowerCase().includes(val.toLowerCase())));
            ...
            const selectCitySearch = (cityName) => {
                ...
                if (coords && mapRef.current) mapRef.current.flyTo(coords, 6, {duration: 2.2, easeLinearity: 0.2});
            };
                ...
                try {
                    if (showVerification) {
                        ...
                        if (data.success) {
                            ...
                            setVerificationCode("");
                            setAuthInput({ email: "", password: "", name: "", avatar: "🌱" });
                    ...
                    if (authMode === 'SIGNUP') {
                        ...
                        // Login fallback
                        const u = { email: authInput.email, name: "Demo User", avatar: "🌱", saved: [] };
                        setUserAccount(u);
                        setShowAuth(false);
            ...
            return (
                <div className={`relative w-screen h-screen overflow-hidden ${view === 'GLOBE' ? 'view-globe' : ''}`}>
                    ...
                    <div className="absolute top-6 right-6 z-[5000] pointer-events-auto">
                    ...
                    <div 
                        className={`absolute top-20 right-6 z-[5000] pointer-events-auto transition-all duration-300 origin-top-right ${showNotifList ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}
                    ...
                    <div className="absolute bottom-6 right-6 z-[5000] flex gap-2 pointer-events-auto">
                        <button onClick={() => setIsSettingCloud(!isSettingCloud)} className={`glass-panel px-4 py-2 rounded-full text-xs font-bold hover:scale-105 transition-transform flex items-center gap-2 ${isCloudConnected ? 'text-emerald-600' : 'text-slate-500'}`}>
                            <IconCloud /> {isCloudConnected ? 'Online' : 'Connect'}
                        </button>
                        <button onClick={() => { setIsSettingHome(!isSettingHome); setHomeInput(""); setHomeSuggestions([]); }} className="glass-panel px-4 py-2 rounded-full text-xs font-bold text-indigo-600 hover:scale-105 transition-transform flex items-center gap-2">
                            <IconLocation /> {homeLocation.name}
                        </button>
                        <button onClick={() => userAccount ? setViewProfile({ ...userAccount, isMe: true, id: userId }) : setShowAuth(true)} className={`glass-panel px-4 py-2 rounded-full text-xs font-bold hover:scale-105 transition-transform flex items-center gap-2 ${userAccount ? 'text-emerald-600' : 'text-slate-600'}`}>
                            {userAccount ? <div className="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center text-[10px]">{userAccount.avatar}</div> : <IconUser />} {userAccount ? userAccount.name : 'Login'}
                        </button>
                    </div>

                    {isSettingHome && (
                        <div className="absolute bottom-20 right-6 z-[5000] glass-panel p-4 rounded-2xl w-72 animate-pop pointer-events-auto">
                            <div className="flex justify-between items-center mb-3">
                                <span className="text-xs font-bold text-slate-500 uppercase">Change Home City</span>
                                <button onClick={() => setIsSettingHome(false)} className="text-slate-400 hover:text-slate-700"><IconX size={14} /></button>
                            </div>
                            <div className="relative">
                                <input autoFocus type="text" placeholder="Type city (e.g. Rome)..." className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400" value={homeInput} onChange={handleHomeChange} />
                                {homeSuggestions.length > 0 && (
                                    <div className="absolute bottom-full left-0 w-full bg-white border border-slate-100 rounded-lg shadow-xl mb-1 max-h-40 overflow-y-auto">
                                        {homeSuggestions.map(city => (<div key={city} className="px-3 py-2 text-xs text-slate-600 hover:bg-indigo-50 cursor-pointer" onClick={() => selectHomeCity(city)}>{city}</div>))}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {isSettingCloud && (
                        <div className="absolute bottom-20 right-40 z-[5000] glass-panel p-4 rounded-2xl w-72 animate-pop pointer-events-auto">
                            <div className="flex justify-between items-center mb-3">
                                <span className="text-xs font-bold text-slate-500 uppercase">Cloud Backend URL</span>
                                <button onClick={() => setIsSettingCloud(false)} className="text-slate-400 hover:text-slate-700"><IconX size={14} /></button>
                            </div>
                            <div className="relative">
                                <input autoFocus type="text" placeholder="wss://..." className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-400 mb-2" value={cloudUrl} onChange={(e) => setCloudUrl(e.target.value)} />
                                <p className="text-[10px] text-slate-400 leading-tight">Deploy the worker, then paste the wss:// URL here to sync global notes.</p>
                            </div>
                        </div>
                    )}
                    ...
                    {viewProfile && (
                        <div className="absolute inset-0 z-[6000] bg-black/30 backdrop-blur-sm flex items-center justify-center pointer-events-auto">
                            <div className="glass-panel p-6 rounded-2xl w-80 animate-pop-in shadow-2xl flex flex-col items-center relative transition-all">
                                ...
                                {viewProfile.isMe && isEditingName ? (
                                    ...
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        {viewProfile.isMe && <button onClick={() => { setIsEditingName(true); setEditNameInput(viewProfile.name); }} className="text-slate-400 hover:text-emerald-600"><IconPen size={14} /></button>}
                                    </div>
                                )}
...
# ---------------------------------------------------------
wrangler_toml_content = r"""
name = "leaf-notes-backend"
main = "src/worker.js"
compatibility_date = "2024-04-05"

[durable_objects]
bindings = [
  { name = "NOTES_ROOM", class_name = "NotesBackend" }
]

[[migrations]]
tag = "v1"
new_sqlite_classes = ["NotesBackend"]
"""
...
create_file(os.path.join(base_path, "wrangler.toml"), wrangler_toml_content)
...
print("2. Run 'deploy.cmd' to push the persistence update to Cloudflare.")
print("3. Run 'publish_github.cmd' to upload to GitHub Pages.")
print("----------------------------------------------------------------")