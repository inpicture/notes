  ...
  async fetch(request) {
    ...
    if (request.method === "POST" && url.pathname.startsWith("/auth/")) {
      ...
      if (url.pathname === "/auth/signup") {
        ...
        users[email] = { password, code, verified: false, created: Date.now(), name: name || "Explorer", avatar: avatar || "\u{1F331}" };
      ...
      if (url.pathname === "/auth/verify") {
        ...
        await this.state.storage.put("users", users);
        return new Response(JSON.stringify({ success: true, user: { email, name: users[email].name, avatar: users[email].avatar } }));
      }
      if (url.pathname === "/auth/login") {
        ...
        return new Response(JSON.stringify({ success: true, user: { email, name: user.name, avatar: user.avatar } }));
      }
    }
